...
export VARIIST_PREVIOUS_VERSION=2.3.0
export VARIIST_VERSION=2.4.0
...
perl -0777 -i \
  -pe "s@$VARIIST_PREVIOUS_VERSION@$VARIIST_VERSION@g;" \
  -pe "s/version = \"[^\"]+-SNAPSHOT\"/version = \"$VARIIST_VERSION\"/;" \
  ./build.gradle.kts
perl -0777 -i \
  -pe 's/(<!-- for main -->\n)\n([\S\s]*?)(\n<!-- for main end -->\n<!-- for release -->\n)<!--\n([\S\s]*?)-->\n(<!-- for release end -->)/$1<!--\n$2-->$3\n$4\n$5/;' \
  -pe 's/(---\n❗ You are taking[^-]*?---)/<!$1>/;' \
...
export VARIIST_GH_PAGES_VERSIONS_JS_VERSION="1.7.0"
export VARIIST_GH_PAGES_VERSIONS_JS_VERSION_NEXT="1.8.0"
...
export VARIIST_NEXT_VERSION=2.4.0
find ./ -name "*.md" | xargs perl -0777 -i \
