...
fun ArbExtensionPoint.fromProgression(progression: IntProgression): ArbArgsGenerator<Int> =
	when (val step = progression.step) {
		...
		-1 -> intFromTo(progression.last, progression.first)
			...
			//TODO 3.5.0 bench at what point it makes sense to calculate it, I just guess that for small progressions
			...
			if (numberOfSteps < 50L) fromList(progression.toList())
				...
				if (numberOfSteps == numberOfStepsI.toLong()) {
					IntFromToArbArgsGenerator(_components, 0, numberOfStepsI).map {
						start + it * stepAbs
					}
				} else {
					LongFromToArbArgsGenerator(_components, 0, numberOfSteps).map {
		...
		-1L -> longFromTo(progression.last, progression.first)
			...
			if (numberOfSteps < 50.toBigInt()) fromList(progression.toList())
				...
				if (numberOfSteps.bitLength() <= 63) {
					LongFromToArbArgsGenerator(_components, 0, numberOfSteps.toLong()).map {
						start + it * stepAbs
					}
				} else {
					BigIntFromToArbArgsGenerator(_components, BigInt.ZERO, numberOfSteps).map {
...
//TODO 3.2.0 introduce BigIntProgression?
