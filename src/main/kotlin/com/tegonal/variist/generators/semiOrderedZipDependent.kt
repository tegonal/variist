//TODO 3.0.0 rename to semiOrderedLike...
 ...
 * with the help of the given [otherFactory] and then [transform]s the value of `this` [SemiOrderedLikeArgsGenerator]
...
fun <A1, A2, R> SemiOrderedLikeArgsGenerator<A1>.zipDependent(
	otherFactory: ArbExtensionPoint.(A1) -> ArbArgsGenerator<A2>,
	transform: (A1, A2) -> R
): SemiOrderedArgsGenerator<R> = mapIndexedInternal { index, a1, seedOffset ->
	val a2 = _components.arb.otherFactory(a1).generateOne(deriveChildSeedOffset(seedOffset, index + 1))
	transform(a1, a2)
}

/**
 * Creates for each generated value of type [A1] by `this` [SemiOrderedLikeArgsGenerator] an [ArbArgsGenerator] with the
 * help of the given [otherFactory], generates the given [amount] of values, and zips them together with
 * the value of `this` [ArbArgsGenerator] to type [Tuple2]`<A1, A2>`.
 *
 ...
 * @param A2 The type of values generated by the other [ArbArgsGenerator] (built by the given [otherFactory]).
 *
 * @return The resulting [SemiOrderedArgsGenerator] which generates values of type [Tuple2]`<A1, A2>`.
 *
 * @since 2.0.0
 */
fun <A1, A2> SemiOrderedLikeArgsGenerator<A1>.flatZipDependent(
	amount: Int,
	otherFactory: ArbExtensionPoint.(A1) -> ArbArgsGenerator<A2>
 ...
 * @since 2.0.0
 */
fun <A1, A2, R> SemiOrderedLikeArgsGenerator<A1>.flatZipDependent(
	amount: Int,
	otherFactory: ArbExtensionPoint.(A1) -> ArbArgsGenerator<A2>,
	transform: (A1, A2) -> R
