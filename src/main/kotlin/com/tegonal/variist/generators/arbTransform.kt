...
/**
 * Maps the values `this` [ArbArgsGenerator] generates to type [R] with the help of the given [transform] function and
 ...
 * @since 3.0.0
 */
fun <T, R> ArbArgsGenerator<T>.mapNotNull(transform: (T) -> R?): ArbArgsGenerator<R> =
	transform { seq -> seq.mapNotNull(transform) }
 ...
 * @param T The type of values generated by `this` [ArbArgsGenerator].
 *
 * @return An [ArbArgsGenerator] which generates chunks of [size] values as a [List].
 *
 * @since 2.0.0
 */
...
/**
 * Instead of emitting the generated values directly, it takes chunks of [size] distinct values,
 * puts them into a [List] and returns it.
 *
 * The values overall are not guaranteed to be distinct, only the values within a chunk are guaranteed to be distinct.
 *
 * @param size the size of the resulting [List]s.
 * @param maxRejections the maximum allowed rejections of duplicates before throwing an [VariistGenerationException].
 *
 * @param T The type of values generated by `this` [ArbArgsGenerator].
 *
 * @return An [ArbArgsGenerator] which generates chunks of [size] distinct values as a [List].
 *
 * @since 3.0.0
 */
fun <T> ArbArgsGenerator<T>.chunkedDistinctValues(
	size: Int,
	maxRejections: Int = 10_000
): ArbArgsGenerator<List<T>> {
	//TODO 3.1.0 once we introduce a SizeAwareArbArgsGenerator we could optimise this if the size of the chunk is,
	// like 40% or more (something to find out) of the size of the ArbArgsGenerator and use Floyd's random sampling
	// algorithm in such a case.
	checkIsPositive(size, "size")
	checkIsPositive(maxRejections, "maxRejections")
			...
			while (values.size < size) {
				...
				if (seen.add(value)) {
					...
					if (rejections > maxRejections) {
						throw VariistGenerationException(
							"Give up to generate a chunk of $size distinct values, already $rejections duplicates rejected"
						)
					}
				}
			}
 ...
 * Note that the resulting sequence must still be an infinite stream of values.
 *
 * @param transform The transformation function which takes a [Sequence] of type [T] as well as a `seedOffset`, which
 *   should be passed along, and produces a [Sequence] of type [R].
 *
 * @param T The type of values generated by `this` [ArbArgsGenerator].
 * @param R the type of values generated by the resulting [ArbArgsGenerator].
 *
